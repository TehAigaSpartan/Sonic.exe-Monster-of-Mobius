...
(function ()
{
				...
				*/
				...
				if (obstacle) {
					...
					if (slope_too_steep || !this.runtime.pushOutSolid(this.inst, -this.downx, -this.downy, push_dist, is_jumpthru, obstacle)){
						...
						//If we haven't yet tried rotating, do so.
						if (!rotated) {
					...
					//Vertical push succeeded, and slope_too_steep is false.
						...
						//If it's invalid, this object should be treated as a wall.
						else {
					...
					if (floor_ && !newFloor && !justJumped) {
						...
						if (this.runtime.testOverlapSolid(this.inst) || this.runtime.testOverlapJumpThru(this.inst)) {
							...
							if (newSurfaceAngle !== null) {
								...
								//REGARDLESS, the second part checks whether the angle is within a certain threshold of the old angle (defaults to ~45 degrees)
								if (this.dx != 0 && cr.angleDiff(this.surfaceAngle, newSurfaceAngle) <= Math.PI*0.26) {
						...
						//If not overlapping it's either a slope that's steeper than 45 degrees or an edge.
							...
							if (!rotated & !justJumped) {
								...
								//this.sonicPhysicsDebugVar += "continue 4, ";
							...
							//We've already tried to rotate, so if it's still not working, give up.
								...
								if (direction == 0) {
									...
									//this.sonicPhysicsDebugVar += this.ga + ", break 6, ";
						...
						//At this stage it's possible we've lined up nicely on a wall. Check the surface angle.
	...
	//NOTE: This function can return null instead of a number. Anywhere you see findSurfaceAngle() || 0, I'm catching the null and replacing it with a 0.
	behinstProto.findSurfaceAngle = function (solidObject)
	{
		//this.sonicPhysicsDebugVar = "fsa, l: " + this.isLeftCornerColliding + ", r: " + this.isRightCornerColliding;
		...
		//if neither is colliding, we MAY want to do something else
		else if (!this.isLeftCornerColliding && !this.isRightCornerColliding) {
			
			//We really only want to do something here if
			
		}
		
		//this.sonicPhysicsDebugVar += "fsa";
		...
		if (solidObject !== null && this.runtime.testOverlap(this.inst, solidObject)) {
			...
			var pointCount;
			//And finally the offset. It'll be 0 if we're using a quad, which uses absolute coords, but a poly is relevant and will need this.
			var solidX, solidY;
			...
			//Update the bounding box and grab the quad
			this.inst.update_bbox();
			...
			//Pretty much all of this bit only needs to happen if we were on a valid corner last tick.
